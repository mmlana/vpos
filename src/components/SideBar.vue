<template>
  <div>
    <v-app-bar-nav-icon fab @click.stop="drawer = !drawer" class="hidden-md-and-up"></v-app-bar-nav-icon>
    <v-navigation-drawer v-model="drawer" app color="white" mini-variant mini-variant-width="90" :permanent="$vuetify.breakpoint.mdAndUp">
      <v-avatar class="d-block text-center mx-auto mt-4" size="40">
        <v-icon color="#ff4b24">fas fa-coffee</v-icon>
      </v-avatar>
      <v-list flat class="mt-4" nav dense>
